...
public:
    ...
    const std::unordered_set<std::string>& GetAttributes() const;
...
public:
    ...
    const std::unordered_map<std::string, std::unique_ptr<Block>>& GetAllBlocks() const;
    ...
    // If there is a name collision, then we will append numbers to the name to make unique.
    ...
    std::string GetUniqueBlockNameInGraph(const std::unique_ptr<Block>& block) const;

    // Here is our collection of blocks in this graph.
private:
    std::unordered_map<std::string, std::unique_ptr<Block>> m_blocks;
    std::unordered_map<std::string, std::vector<Block*>> m_attributes;
};
