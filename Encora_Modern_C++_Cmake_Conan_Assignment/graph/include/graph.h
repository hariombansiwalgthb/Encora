...
public:
    ...
    std::unordered_set<std::string>& GetAttributes();
private:
    std::string m_name;
...
struct Graph
{
    ...
    std::unordered_set<Block*> GetBlocks();

    std::unordered_set<Block*> GetBlocksWithAttribute(const std::string& attribute);
    ...
    std::string GetUniqueBlockNameInGraph(const std::string& baseName);

private:
    std::unordered_map<std::string, std::unique_ptr<Block>> m_blocksByName;
    std::unordered_multimap<std::string, Block*> m_attributeIndex;
};
